...
import {printNode, Project, ScriptKind, ts, VariableDeclarationKind} from "ts-morph";
import {logCompletion, logError, logInfo} from "./log";
import {allowAll} from "@keystone-6/core/access";

const options = yargs
.usage("Usage: -f <prisma-file-path>")
.option("f", {alias: "file", describe: "Prisma file path", type: "string", demandOption: true})
.option("o", {
  alias: "output",
...
function normalizeAllCapsToHumanReadable(allCapsString) {
  ...
  allCapsString = allCapsString.split(" ").map(
...
function getObjectForField(field, enums) {
  ...
  const dbs = [
    ts.factory.createPropertyAssignment(
      ...
      field.isRequired ? ts.factory.createFalse()
  ...
  if (field?.kind?.toLowerCase() === 'scalar') {
    switch (field.type.toLowerCase()) {
      case 'string': {
        fieldFunctionType = field.name.toLowerCase() === 'password' ? 'password' : 'text';
      ...
      case 'boolean':
        ...
        if (field.hasDefaultValue && typeof field.default === 'boolean') {
          innerObject.push(
            ts.factory.createPropertyAssignment(
              ts.factory.createIdentifier("defaultValue"),
              field.default === true ? ts.factory.createTrue() : ts.factory.createFalse()
            )
          )
        }
        break;
      case 'datetime':
        ...
        dbs.push(
          ts.factory.createPropertyAssignment(
            ...
            field.isUpdatedAt ? ts.factory.createTrue()
    ...
    innerObject = [
      ...
      ts.factory.createPropertyAssignment(
        ...
        ts.factory.createArrayLiteralExpression(
          enums.get(field.type).map(option => ts.factory.createObjectLiteralExpression([
            ts.factory.createPropertyAssignment(
              ts.factory.createIdentifier("label"),
              ts.factory.createStringLiteral(normalizeAllCapsToHumanReadable(option))
            ),
            ts.factory.createPropertyAssignment(
  ...
  if (fieldFunctionType !== 'relationship' &&
  ...
  return (
    ts.factory.createPropertyAssignment(
      ...
      ts.factory.createCallExpression(
        ...
        undefined, innerObject?.length > 0 ? [
...
async function writeTSFile(parsed) {
  ...
  const initial = [
    ...
    `
            // Allow access to keystone. This is default, but you should update to
            // fit your needs.
            import { allowAll } from "@keystone-6/core/access";
        `,
  ...
  const sourceFile = project.createSourceFile(options.output, (writer) => {
    initial.map(item => writer.write(item.replace(/^[\s]{12}/gm, '')).blankLine())
  ...
  parsed.enums.forEach(item => enumNameToValues.set(item.name, item.values.map(x => x.name)))
  ...
  if (options.enum) {
    for (let e of enumNameToValues.keys()) {
      ...
      en.addMembers(enumNameToValues.get(e).map(item => {
        return {name: item}
      }));
    }
            ...
            undefined, [
              ts.factory.createObjectLiteralExpression([
                ts.factory.createPropertyAssignment(
                  ...
                  ts.factory.createIdentifier("allowAll"),
                ),
                ts.factory.createPropertyAssignment(
                  ts.factory.createIdentifier("fields"),
                  ts.factory.createObjectLiteralExpression(
                    modelNameToFieldsMap.get(model).filter(f => f.name !== 'id').map(
                      field => getObjectForField(field, enumNameToValues)
                    ), true
                  ),
                ),
              ], true),
